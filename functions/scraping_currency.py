...
#     """
#     short_title = soup.find("small", class_="nameSymbol").get_text
#     return short_title
...
#     # with open(f"data/{day}/{time}.json") as write_file:
#     #     data = json.load(write_file)

#     for item in crypto_currency:
#         crypto = f'Название: {item.get("title")}\n' \
#         f'Сокращение: {item.get("information").get("short_title")}\n' \
#         f'💵 Цена: {item.get("information").get("price")}\n' \
#         f'📉 Низкая: {item.get("information").get("low_high_24h").get("low")}\n' \
#         f'📈 Высокая: {item.get("information").get("low_high_24h").get("high")}\n' \
#         f'Рыночная цена: {item.get("information").get("market_price")}\n' \
#         f'Сайт: {item.get("information").get("website")}\n' \
#         f'Картинка: {item.get("information").get("img")}\n' \
#         f'Путь: {item.get("information").get("path")}'
#         # await message.answer(crypto, reply_markup=await keyboard())